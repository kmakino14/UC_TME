...
#################################################
# Fig.S5C  Functional genes heatmap
#################################################
...
paste0('Reproducibility/Results/Plots/Myeloid/FigureS5C.pdf') %>% pdf(.,w = 15, h = 3)
 draw(ht1+ht2+ht3+ht4+ht5+ht6+ht7+ht8+ht9+ht10)
dev.off()


#################################################
# Fig.S5D  Signature violin plot
#################################################
...
vlnplot_w_box(Seurat = DOGMA_part,
              ...
              file_name = "Reproducibility/Results/Plots/Myeloid/FigureS5D.pdf", 
              cols = cols)


#################################################
# Fig.S5E  Milo beeswarm plot
#################################################
...
paste0("Reproducibility/Results/Plots/Myeloid/FigureS5E.pdf") %>% pdf(.,width = 14, height = 5)
 p1+p2
dev.off()


#################################################
# Fig.S5F  eRegulon score box plot
#################################################
...
pdf("Reproducibility/Results/Plots/Myeloid/FigureS5F.pdf", width = 10, height = 6)
 print(p)
dev.off()


#########################################################
## Fig.S5G  Gene track with co-accessible regions
#########################################################
source("Reproducibility/Scripts/Source/Seurat_source.R")
...
Plot_genetrack(Obj=DOGMA, group.by = 'celltype', lineage='Myeloid',
               ...
               path='Reproducibility/Results/Plots/Myeloid/FigureS5G_CD86.pdf')
...
Plot_genetrack(Obj=DOGMA,group.by = 'celltype', lineage='Myeloid',
               ...
               region_list=region_list, motif_list=motif_list,cutoff=0.2,cols=cols,
               path='Reproducibility/Results/Plots/Myeloid/FigureS5G_VEGFA.pdf')