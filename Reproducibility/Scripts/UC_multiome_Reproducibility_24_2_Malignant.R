...
#################################################
# Fig.S2F  Ridgeplot F3, CD142
#################################################
cols = rev(c('#533f7c','#37888e','#4c8d49','#ce8d24','#aa4743'))
DOGMA$celltype = factor(DOGMA$celltype, levels = rev(c("NEC","LUM","MES","SQM","NRP")))
p = RidgePlot(object = DOGMA, assay = 'RNA', cols = cols, 
              features = c("F3"), 
              group.by = 'celltype')
p_zoomed = p + coord_cartesian(xlim = c(0, 0.2))

q = RidgePlot(object = DOGMA, assay = 'ADT', cols = cols, 
              features = c("surface-A0822-CD142"), 
              group.by = 'celltype', sort = TRUE)

paste0('Reproducibility/Results/Plots/Malignant/FigureS3F.pdf') %>% pdf(., w = 4, h = 6)
 plot(p_zoomed/q)
dev.off()
