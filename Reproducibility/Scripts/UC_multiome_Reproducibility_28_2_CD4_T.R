...
#################################################
# Fig.S9C  Signature score heatmap (Primary)
#################################################
...
paste0('Reproducibility/Results/Plots/CD4_T/FigureS9C.pdf') %>% pdf(.,w = 5, h = 5)
 draw(ht)
dev.off()


#################################################
# Fig.5C  Milo beeswarm plot
#################################################
...
paste0("Reproducibility/Results/Plots/CD4_T/Figure5C.pdf") %>% pdf(.,width = 14, height = 5)
 p1+p2
dev.off()


#################################################
# Fig.5D  Treg fate probability
#################################################
...
paste0("Reproducibility/Results/Plots/CD4_T/Figure5D_fate_prob_UMAP_w_bg.pdf") %>% pdf(w=3, h=5)
 plot(p)
dev.off()

#################################################
# Fig.S9E  eRegulon score box plot
#################################################
...
pdf("Reproducibility/Results/Plots/CD4_T/FigureS9E.pdf", width = 7, height = 9)
 print(p)
dev.off()


#########################################################
## Fig.S9D/F  Gene track with co-accessible regions
#########################################################
source("Reproducibility/Scripts/Source/Seurat_source.R")
...
Plot_genetrack(Obj=DOGMA,group.by='celltype',lineage='CD4_T',group_list=celltype_keep,gene='IL17A',
               ...
               path='Reproducibility/Results/Plots/CD4_T/FIgureS9D.pdf')
...
Plot_genetrack(Obj=DOGMA,group.by='celltype',lineage='CD4_T',group_list=celltype_list,gene='CCR8',
               region_list=region_list, motif_list=motif_list,cutoff=0.3,cols=cols,
               path='Reproducibility/Results/Plots/CD4_T/FIgureS9F.pdf')



#########################################################
## Fig.5F  Gene track with co-accessible regions
#########################################################
...
Plot_genetrack(Obj=Treg,group.by='state',lineage='CD4_T',group_list=group_list,gene='CXCR3',
               ...
               path='Reproducibility/Results/Plots/CD4_T/FIgure5F.pdf')


#########################################################
## Fig.S9H  Heatmap of functional genes
#########################################################
...
paste0('Reproducibility/Results/Plots/CD4_T/FigureS9H.pdf') %>% pdf(.,width = 13, height = 2)
 draw(ht1+ht2+ht5+ht6+ht3+ht4)
dev.off()


#########################################################
## Fig.S9I  Differential analysis between Treg states
#########################################################
# RNA
...
pdf("Reproducibility/Results/Plots/CD4_T/FigureS9K.pdf", width = 4, height = 10)
 print(p1/p2)
dev.off()


#########################################################
## Fig.5G  Differential analysis in eTregs
#########################################################
# RNA
...
paste0("Reproducibility/Results/Plots/CD4_T/Figure5G_RNA.pdf") %>% pdf(.,width=3,height=3)
...
paste0("Reproducibility/Results/Plots/CD4_T/Figure5G_ADT.pdf") %>% pdf(.,width=3,height=3)
...
paste0("Reproducibility/Results/Plots/CD4_T/Figure5G_TF.pdf") %>% pdf(.,width=3,height=3)
  Volcano_DEG_label_v2(data = data3, celltype = "MI enr nhoods", label_up = label_up, label_down = label_down, ylim = 100, log2FC = 0.5)
dev.off()

#########################################################
## Fig.5H/5I/S9L  CCI analysis (CellChat)
#########################################################
...
paste0('Reproducibility/Results/Plots/CD4_T/FIgure5H.pdf') %>% pdf_3(., w=12, h=6)
...
weight.max <- getMaxWeight(object.list, slot.name = c("netP"), attribute = pathways.show) # control the edge weights across different datasets
paste0('Reproducibility/Results/Plots/CD4_T/FIgureS9L.pdf') %>% pdf_3(., w=12, h=6)
par(mfrow = c(1,2), xpd=TRUE)
for (i in 1:length(object.list)) {
  netVisual_aggregate(object.list[[i]], signaling = pathways.show, layout = "circle", edge.weight.max = weight.max[1], 
                      edge.width.max = 10, signaling.name = paste(pathways.show, names(object.list)[i]))
...
paste0('Reproducibility/Results/Plots/CD4_T/FIgure5I.pdf') %>% pdf_3(., w=4, h=3)
 gg1
dev.off()
