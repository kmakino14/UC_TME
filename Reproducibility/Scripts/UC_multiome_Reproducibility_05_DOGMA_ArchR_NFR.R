...
SAMPLE_TMP = 'UC_DOGMA_reseq'

######################################################
# Create Arrow files
######################################################

fragment_path  = paste0("Reproducibility/Data/ATAC_fragments/atac_fragments_Global.tsv.gz")   #### cellranger-atac aggr output after subsetting from ARC

dir_list = c(paste0("Reproducibility/Results/ArchR/Global/output/log"), 
             paste0("Reproducibility/Results/ArchR/Global/output/QC")
             )
...
ArrowFiles = createArrowFiles(
  ...
  force = TRUE,
  QCDir   = "Reproducibility/Results/ArchR/Global/output/QC", 
  logFile = "Reproducibility/Results/ArchR/Global/output/log/UC_DOGMA_createArrowFiles.log"
)

proj = ArchRProject(
  ArrowFiles = ArrowFiles, 
  outputDirectory = paste0("Reproducibility/Results/ArchR/Global/output"),
...
ArchR_preprocess = function(tmp_lineage){
    ...
    df = dplyr::filter(metadata, lineage %in% c(tmp_lineage))
    ...
    proj = loadArchRProject(path = paste0("Reproducibility/Results/ArchR/Global/output"),
                            force = FALSE, showLogo = FALSE)
    
    ## cell barcode after QC filter by Seurat
    target_cell = intersect(paste0(SAMPLE_TMP,"#",rownames(df)),rownames(proj))
    ...
    target_cell = intersect(paste0(SAMPLE_TMP,"#",rownames(df)),rownames(proj_subset))
    df$CB = paste0(SAMPLE_TMP,"#",rownames(df))
    Single_cell_list = left_join(data.frame(CB=target_cell), df ,by="CB")
    proj_subset$celltype = Single_cell_list$celltype
...
ArchR_preprocess(tmp_lineage = 'CD4_T')
ArchR_preprocess(tmp_lineage = 'CD8_T_NK_ILC')
ArchR_preprocess(tmp_lineage = 'B')
ArchR_preprocess(tmp_lineage = 'Myeloid')
ArchR_preprocess(tmp_lineage = 'MSC')